...
class RegisterController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        return redirect()->route('user-register')->with('sendmail', 'Đăng ký tài khoản thành công đăng nhập để sửu dụng dịch vụ');
        // Send Mail to Buyer
        ...
        if ($be->is_smtp == 1) {
            try {
                ...
                return redirect()->route('user-register')->with('sendmail', 'A verifican mail has been sent to your email address.');
            } catch (Exception $e) {
                return back()->with('sendmail', 'Send mail errors');
            }
        } else {
            try {
